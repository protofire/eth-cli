...
describe('running eth randomAddress', () => {
  it('should print a random address with its private key', async () => {
    const result = shell.exec(`${binPath} randomAddress`, { silent: true })
  ...
  it('should print two random addresses with their private key', async () => {
    const result = shell.exec(`${binPath} randomAddress 2`, { silent: true })
  ...
  it('should print different addresseses with their private key', async () => {
    const result = shell.exec(`${binPath} randomAddress 3`, { silent: true })
...
function parse(output) {
  ...
  const outputJSON = outputReplaced.slice(0, lastBrace) + '}' + outputReplaced.slice(lastBrace + 2)
